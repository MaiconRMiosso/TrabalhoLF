...
function addWordTocollection () {
    var value = $("#word").val().toLowerCase(); // retorna a palavra em letra minuscula

    if(value === ""){  // se for vazio da erro
      $('#word').addClass('error');
      setTimeout(function(){
      ...
      var addNext = true;
      //verifica se no token a ser adicionado tem algum caractere que nao seja de 'a' a 'z' ou espaço
        ...
        if(value.length > 1){
          for (i = 0; i < value.length; i++) {
            ...
            if(value[i] !== ""){
              ...
              if(!exists){
                $('#listWords').append($('<div class="tabela" id="word' + number + '">' + value[i] + ', ' +
                ' </div>'));
          ...
          //se nao existir no dicionario e adicionado
          if(!exists){
            $('#listWords').append($('<td class="tabela" id="word' + number + '">' + value[0] +
            ' </td>'));
...
function removeWord (e) {
  ...
  for (i = 0; i < collection.length; i++) {
    $('#listWords').append($('<td class="tabela" id="word' + i + '">' + collection[i] +
    '</td>'));
  }
...
//metodo gerar a tabela visível no front 
